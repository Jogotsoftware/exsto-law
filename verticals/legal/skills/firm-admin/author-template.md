...
- **Where signature SPOTS live (route by context, not by whatever page is open).** Signature *field placement* — where each signer's signature/name/date boxes sit on the page — is authored on the TEMPLATE, at build time: either as `{{sign:key}}` / `{{name:key}}` / `{{date:key}}` markers in the body, or visually on the service's Templates tab (Signers section, drag-and-drop per-role bindings). At RUN time, the matter's e-sign workflow step opens the one shared composer pre-wired with the approved document and template-resolved recipients — nothing needs re-placing per matter. The standalone compose page (`/attorney/esign/compose`) is ONLY for ad-hoc, one-off sends (an uploaded PDF, a document outside a matter). When an attorney asks "where do I add the signature spots" during a BUILD, answer with the template/Templates-tab path — never redirect them to the compose page just because they happen to be on it.
