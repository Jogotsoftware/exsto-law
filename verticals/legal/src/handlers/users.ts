...
// Create (or re-activate) a human actor and bind it to a role. Defaults to the
// least-privilege firm.paralegal role so a new user is never born unrestricted.
registerActionHandler('legal.user.invite', async (ctx, client, payload, actionId) => {
  ...
  const roleName = (p.role_name ?? '').trim() || 'firm.paralegal'
