...
registerTool({
  ...
  description:
    'Fetch the firm profile (firm name, mailing address, phone, contact email, home jurisdiction, practice areas, attorney name, assistant instructions, client portal instructions, public-page tagline and about paragraph) — the identity block generated documents and letterheads resolve, plus the firm jurisdiction fallback for matters with no per-matter override, the firm-wide standing instructions the internal AI assistant (attorney chat + AI-drafted email) follows, the separate client-safe instructions the CLIENT PORTAL assistant follows, and the copy shown on the firm public landing page. Values come from the firm_profile record, falling back to legacy settings where never set.',
  mode: 'read',
...
registerTool({
  ...
  description:
    'Set the firm profile (firm name, mailing address, phone, contact email, home jurisdiction, practice areas, attorney name, assistant instructions, client portal instructions, public-page tagline/about, and branding: brand color, secondary brand color, firm logo, header logo). Undefined fields are left alone; an empty value clears the field. Each change is appended — prior values stay in history. Firm jurisdiction accepts a US state code or name (e.g. "NC" or "North Carolina") and is rejected if unrecognized. Assistant instructions are free text (e.g. "always CC my paralegal"), injected into the INTERNAL AI assistant chat and drafted emails — never the client portal. Client portal instructions are a SEPARATE free-text field (e.g. "mention our office closes at 5pm"), injected only into the CLIENT-FACING portal assistant. Both cap at 2,000 characters. `headerColor` and `secondaryColor` are #rrggbb display colors (empty clears): the first is the firm’s main brand color, the second overrides the companion/accent tone that is otherwise derived by darkening the first. `logoDataUrl` and `logoSecondaryDataUrl` are image data URLs (PNG/JPG/GIF/WEBP, under 500 KB): the first is the firm-wide mark used on the portal, booking page, public page and invoices; the second is shown only on the attorney console header bar. `tagline` (max 160 chars) and `about` (max 4,000 chars) are the PUBLIC copy on the firm landing page — client-visible marketing text, never internal instructions.',
  mode: 'write',
...
registerTool({
  ...
  description:
    "The firm's visual identity for chrome: firm name, brand color and secondary brand color (#rrggbb or null — null means the companion tone stays derived from the primary), the firm logo as an image data URL (or null), and the optional HEADER logo shown only on the attorney console top bar. The firm logo resolves from the firm profile, falling back to a logo uploaded on the older invoice-template config where the firm has never set one.",
  mode: 'read',
