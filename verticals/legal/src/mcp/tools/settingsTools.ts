...
registerTool({
  ...
  description:
    'Set the firm profile (firm name, mailing address, phone, contact email, home jurisdiction, practice areas, attorney name, assistant instructions, client portal instructions, public-page tagline/about). Undefined fields are left alone; an empty value clears the field. Each change is appended — prior values stay in history. Firm jurisdiction accepts a US state code or name (e.g. "NC" or "North Carolina") and is rejected if unrecognized. Assistant instructions are free text (e.g. "always CC my paralegal"), injected into the INTERNAL AI assistant chat and drafted emails — never the client portal. Client portal instructions are a SEPARATE free-text field (e.g. "mention our office closes at 5pm"), injected only into the CLIENT-FACING portal assistant. Both cap at 2,000 characters. `tagline` (max 160 chars) and `about` (max 4,000 chars) are the PUBLIC copy on the firm landing page — client-visible marketing text, never internal instructions.',
  mode: 'write',
