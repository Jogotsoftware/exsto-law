...
// The same audit surface aggregated across a SET of matters — the client/contact
// Activity tab (all activity on every matter that person is on). Same two streams
// as getMatterHistory, matched by matter = ANY(...), merged newest-first-agnostic
// (the caller sorts). Empty set → empty history (no query).
export async function getHistoryForMatters(
  ctx: ActionContext,
  matterEntityIds: string[],
): Promise<MatterHistory> {
  if (matterEntityIds.length === 0) return { actions: [], events: [] }
  return withActionContext(ctx, async (client) => {
    const actions = await client.query<{
      ...
      `SELECT a.id AS action_id, akd.kind_name, a.intent_kind, a.autonomy_tier,
       ...
       WHERE a.tenant_id = $1 AND (
         a.payload->>'matter_entity_id' = ANY($2::text[])
         ...
         OR a.id::text IN (
           ...
           WHERE a2.tenant_id = $1 AND a2.payload->>'matter_entity_id' = ANY($2::text[])
             AND a2.payload->>'intake_action_id' IS NOT NULL
         )
       )
       ORDER BY a.recorded_at ASC`,
      [ctx.tenantId, matterEntityIds, matterEntityIds],
    )
    ...
    const events = await client.query<{
      ...
      `SELECT e.id AS event_id, ekd.kind_name, e.payload,
       ...
       WHERE e.tenant_id = $1
         AND (e.primary_entity_id = ANY($2::uuid[]) OR e.secondary_entity_ids && $2::uuid[])
