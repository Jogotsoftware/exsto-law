...
// Build the Claude system text: the base prompt + the matter/client context, the
// current entity's in-app link (Claude is the firm's own model, so the link/id is
// safe; the external research path never receives it), the attorney-selected
// active skills, and the skills catalog.
...
export async function assistantChat(
  ...
  if (model.provider === 'perplexity') {
    ...
    // Claude: full matter context is safe (the firm's own model). Load the skill
    // catalog so the model can pull a playbook on demand (load_skill), plus any
    // skills the attorney force-selected from the /skills menu.
