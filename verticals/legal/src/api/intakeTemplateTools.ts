...
const QUESTIONNAIRE_CONTEXT_TOOL_DEF = {
  ...
  description:
    "Get everything needed to PROPOSE the intake QUESTIONNAIRE for an existing service: the closed set of field types you may use, the service's current questionnaire (null if none), the {{tokens}} its document templates reference (your questionnaire SHOULD collect a field for each so the documents merge with no [[MISSING]] gaps), and the firm's saved questionnaires you can reuse. Compose a questionnaire ONLY from those field types, and prefer field ids that MATCH the template tokens so the form covers the documents. Call this FIRST whenever the attorney asks you to build, add to, or change a service's intake form.",
  input_schema: {
    type: 'object',
    properties: {
      service_key: {
...
const TEMPLATE_CONTEXT_TOOL_DEF = {
  ...
  description:
    "Get everything needed to PROPOSE a document TEMPLATE for an existing service: the service's current questionnaire field ids (use these EXACT ids as your {{tokens}} so the document merges with no [[MISSING]] gaps), its existing document templates, and the document-kind registry. Every {{token}} you put in the body should match one of those field ids; a token with no matching field is an ORPHAN that renders [[MISSING]]. Tokens are flat snake_case — NEVER dotted paths. Call this FIRST whenever the attorney asks you to draft, add, or change a service's document template.",
  input_schema: {
    type: 'object',
    properties: {
      service_key: {
