...
// Clients list + in-app create (WP2.2). A client is the billing parent that
// groups contacts and matters. Creating one fires legal.client.create through
// the core (the first place an attorney can add a CRM record by hand — contacts
// otherwise only arrive via intake). Each row links to the client page.
...
export default function ClientsPage() {
  ...
  async function create() {
    ...
    try {
      ...
      if (newId) {
        router.push(`/attorney/clients/${newId}`)
        return
      }
        ...
        <div className="matter-list">
          {clients.map((c) => (
            <Link
              ...
              href={`/attorney/clients/${c.clientEntityId}`}
