...
// The attorney calendar (WP7, REQ-CALMAIL-01; UIWALK-1): one component shared
// by the Calendar page and the home page (embedded), so both render the exact
// same calendar. Day / week / month / list views with in-app create / edit /
// duplicate / cancel that write through the action layer and round-trip to
// Google. EVERY event is editable — app-managed consultations/meetings via
// their actions, and the attorney's other Google events via the generic
// legal.calendar.event_update / event_delete write-through tools. Tasks with a
...
interface WorkspaceEvent {
  ...
  status: string
  // Which of the attorney's calendars the event lives on — generic writes
  // (legal.calendar.event_update / event_delete) must target it.
...
// A guest-email chips input with suggestions (firm contacts + addresses seen on
// recent calendar events). Typing filters the suggestions; Enter/comma adds a
// raw address; clicking a suggestion adds it. Chips remove on ×.
function EmailChipsInput({
  ...
  suggestions: Array<{ label: string; email: string }>
  placeholder?: string
}) {
  ...
  const matches = suggestions
    .filter((sug) => !chosen.has(sug.email.toLowerCase()))
    .filter(
      (sug) => !q || sug.email.toLowerCase().includes(q) || sug.label.toLowerCase().includes(q),
    )
    .slice(0, 6)
  function add(raw: string) {
    const e = raw.trim().replace(/,$/, '')
    if (!e || !e.includes('@') || chosen.has(e.toLowerCase())) return
    onChange([...value, e])
    setText('')
  }
  return (
    <div className="li-cal-chipsinput">
      <div className="li-cal-chipsinput-box">
        {value.map((e) => (
          <span key={e} className="li-cal-guestchip">
            {e}
            <button
              type="button"
              aria-label={`Remove ${e}`}
        ...
        <input
          type="text"
          value={text}
          placeholder={value.length === 0 ? (placeholder ?? 'guest@example.com') : undefined}
          ...
          onKeyDown={(ev) => {
            if (ev.key === 'Enter' || ev.key === ',') {
              ev.preventDefault()
              add(text)
            } else if (ev.key === 'Backspace' && !text && value.length > 0) {
      ...
      {focused && matches.length > 0 && (
        <div className="li-cal-chipsinput-menu" role="listbox">
          {matches.map((sug) => (
            <button
              key={sug.email}
              type="button"
            ...
            >
              <span className="li-cal-chipsinput-name">{sug.label}</span>
              <span className="li-cal-chipsinput-email">{sug.email}</span>
            </button>
          ))}
        </div>
      )}
...
type Panel =
  | {
      ...
      durationMin: number
      categoryKey: string
      // Attach a Google Meet link to the new event.
      includeMeet: boolean
      // Extra guest emails to invite (chips; suggestions from contacts + recent).
      guests: string[]
    }
  | {
      kind: 'edit'
      e: WorkspaceEvent
      // Editable ONLY for the attorney's plain Google events (no app entity) —
      // app-managed consultations/meetings keep their server-derived titles.
      summary: string
      date: string
      time: string
      durationMin: number
...
export function CalendarWorkspace({ embedded = false }: { embedded?: boolean }) {
  ...
  // Guest-email suggestions: firm contacts first, then any other address seen
  // on the loaded window's events ("recent emails").
  const guestSuggestions = useMemo(() => {
    const out: Array<{ label: string; email: string }> = []
    const seen = new Set<string>()
    for (const c of contacts) {
      if (!c.email || seen.has(c.email.toLowerCase())) continue
      seen.add(c.email.toLowerCase())
      out.push({ label: c.fullName || c.email, email: c.email })
    }
    for (const e of events) {
      for (const email of e.attendeeEmails) {
        if (seen.has(email.toLowerCase())) continue
        seen.add(email.toLowerCase())
        out.push({ label: email, email })
      }
    }
    return out
  }, [contacts, events])
  ...
  // WP-H) — the attorney can then drag it to a new slot. A plain Google event
  // duplicates as an app-created meeting (same title/time).
  async function duplicateEvent(e: WorkspaceEvent) {
    ...
    if (e.matterEntityId) {
      ...
      await run('legal.meeting.create', {
        summary: e.summary === '(no title)' ? 'Event' : e.summary,
        startIso: e.startIso,
        endIso,
      })
    }
  ...
  function deleteEvent(e: WorkspaceEvent) {
    ...
    const isConsultation = !isMeeting && Boolean(e.matterEntityId)
    ...
    void confirm({
      ...
      body: isMeeting
        ...
        : isConsultation
          ...
          : `Deletes “${e.summary || '(no title)'}” from your Google calendar. Guests are notified.`,
      confirmLabel: isMeeting
        ? 'Cancel meeting'
        : isConsultation
          ? 'Cancel consultation'
          : 'Delete event',
      ...
      if (isMeeting) {
        ...
        run('legal.calendar.event_delete', {
          googleEventId: e.eventId,
          ...(e.calendarId ? { calendarId: e.calendarId } : {}),
        })
      }
    })
  }
  ...
  function eventMenuItems(e: WorkspaceEvent): ActionItem[] {
    if (!e.meetingEntityId && !e.matterEntityId) {
      // One of the attorney's plain Google events — editable via the generic
      // write-through tools.
      const items: ActionItem[] = [{ label: 'Edit Event', onClick: () => openEdit(e) }]
      if (e.htmlLink) items.push({ label: 'Open in Google', href: e.htmlLink })
      items.push({ label: 'Delete Event', danger: true, onClick: () => deleteEvent(e) })
      return items
    }
  ...
  async function submitPanel() {
    ...
    try {
      if (panel.kind === 'create') {
        const guests = panel.guests.map((g) => g.trim()).filter((g) => g.includes('@'))
        if (panel.mode === 'matter') {
          ...
          await callAttorneyMcp({
            ...
            input: {
              ...
              startIso,
              endIso,
              includeMeet: panel.includeMeet,
              ...(guests.length > 0 ? { attendeeEmails: guests } : {}),
            },
          })
        ...
        if (e.meetingEntityId) {
          ...
          // A plain Google event: title and/or time via the generic write-through.
          const newSummary = panel.summary.trim()
          await callAttorneyMcp({
            toolName: 'legal.calendar.event_update',
            input: {
              googleEventId: e.eventId,
              startIso,
              endIso,
              ...(newSummary && newSummary !== e.summary ? { summary: newSummary } : {}),
              ...(e.calendarId ? { calendarId: e.calendarId } : {}),
            },
          })
        }
  ...
  async function submitAttendees() {
    ...
    const emails = attendeeEmails.map((s) => s.trim()).filter((s) => s.includes('@'))
  ...
  async function rescheduleEventTo(e: WorkspaceEvent, startIso: string, endIso: string) {
    if (e.meetingEntityId) {
      ...
      await run('legal.calendar.event_update', {
        googleEventId: e.eventId,
        startIso,
        endIso,
        ...(e.calendarId ? { calendarId: e.calendarId } : {}),
  ...
  // A positioned event block in the hourly grid. EVERY event is draggable while
  // the live Google read is up — drag the body to reschedule, drag the bottom
  // edge to change the end. Matter blocks deep-link to the matter on a plain
  // click; everything else opens the edit modal. The pencil + right-click both
  // open the context menu (Edit / Duplicate / Delete) — the attorney's plain
  // Google events write back via legal.calendar.event_update/event_delete.
  // Color = call-type palette, falling back to the navy "managed" bar.
  // `leftPct`/`widthPct` come from layoutTimed's side-by-side lane split.
  function renderGridEvent(
    ...
    const color = eventColor(e)
    // UIWALK-1: every event with a real start time is draggable/editable — the
    // attorney's plain Google events write back via legal.calendar.event_update.
    const draggable = source === 'google'
    const d = drag
    ...
    if (draggable) {
      // A contact/personal meeting or a plain Google event — draggable; a plain
      // click opens the edit modal (there's no matter to deep-link to).
      return (
        <div
          key={e.eventId}
          className={cls}
    ...
    <div className="li-cal-root">
      ...
      <div className="li-cal-header">
        <div className="li-cal-header-left">
          {!embedded && <h1 className="li-cal-title">Calendar</h1>}
      ...
      {view === 'week' && <section>{renderTimeGrid(period.days)}</section>}
        ...
        <div className="li-cal-modal-backdrop" onClick={() => !busy && setPanel(null)}>
          <div className="li-cal-modal" onClick={(ev) => ev.stopPropagation()}>
            ...
            <div className="li-cal-modal-body">
              ...
              {panel.kind === 'edit' &&
                (panel.e.matterEntityId || panel.e.meetingEntityId ? (
                  <label className="li-cal-field">
                    <span>Title</span>
                    <input
                      type="text"
              ...
              {panel.kind === 'edit' &&
                !panel.e.matterEntityId &&
                !panel.e.meetingEntityId &&
                panel.e.htmlLink && (
                  <label className="li-cal-field">
                    <span>Google</span>
                    <a
                      href={panel.e.htmlLink}
                      target="_blank"
                      rel="noreferrer"
              ...
              {panel.kind === 'create' && (
                <>
                  <label className="li-cal-field">
                    ...
                    <EmailChipsInput
                      value={panel.guests}
                      onChange={(guests) => setPanel({ ...panel, guests })}
                      suggestions={guestSuggestions}
                      placeholder={
                        panel.mode === 'matter'
                          ? 'Extra guests beyond the client…'
                          : 'Add guest emails…'
                      }
                    />
                  </label>
                  <label className="li-cal-field li-cal-field-check">
                    <input
                      type="checkbox"
                      checked={panel.includeMeet}
                      onChange={(e) => setPanel({ ...panel, includeMeet: e.target.checked })}
                    />
                    <span>Include a video call (Google Meet link)</span>
                  </label>
                </>
              )}
      ...
      {attendeesFor && (
          ...
          footer={
            <>
              ...
              <button
                ...
                disabled={busy || attendeeEmails.length === 0}
        ...
        >
          <p className="li-modal-muted" style={{ marginTop: 0 }}>
            Add guests to this consultation. Google emails them the invite.
          </p>
          <EmailChipsInput
