...
**Keep it current.** This is a living document, not an append-only log — when something here ships, gets fixed, or changes, EDIT this file in place (update the section, don’t just add a new dated bullet at the bottom). Last updated: 2026-08-03.
...
6. ~~**The workflow's e-sign step has no real in-place UI.**~~ **TRACED FALSE 2026-08-03 (ESIGN-STEP-1, PR #511, `docs/diagnostics/ESIGN-STEP-1-TRACE.md`).** The pop-out modal pre-wired with the approved document and template-resolved recipients has existed since #451 — the step opens the one shared EsignComposer in place. The reported dead end traced to two other things: (a) the step had never actually run (no matter was ever opened on the multi-member service), and (b) asked "where do I add the signature spots" mid-BUILD, the AI assistant deferred to the standalone compose page the attorney happened to be on instead of explaining that fields are placed on the template at build time and the step modal takes over at run time — an assistant-guidance gap logged in INVENTORY.md §6. Also retired: a stale `send_document_for_e_signature` capability request filed 2026-07-20 while the Pacheco capability catalog was still unseeded. Both authoring and runtime already refuse/flag workflows naming nonexistent capabilities (proven live); a registry sweep found no orphan slugs in any tenant. Still true and open: the *single-member* LLC service has no e-sign step at all (signing is a manual to-do) — adding one is a one-step workflow edit awaiting the founder's go-ahead.
